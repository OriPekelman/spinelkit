# SpinelKit::Json -- the union of toy's Toy::Json and tep's Tep::Json.
#
# WHY THIS EXISTS. Spinel (the Ruby->native AOT compiler) cannot lower the
# stdlib `json` gem: its fast path is a CRuby C extension (`JSON::Ext`) and
# the pure-Ruby fallback (`JSON::Pure`) is heavily metaprogrammed
# (`define_method` state transitions, `class_eval`'d generator dispatch).
# `oj`/`yajl-ruby`/`multi_json` are all C extensions or thin wrappers. The
# spinelgems catalog confirms it: `json` is rejected, `oj` risky -- there is
# NO verified pure-Ruby JSON gem to reuse. So toy and tep each hand-rolled
# one, and their escape/quote/hex code came out byte-identical. This file is
# that code, once.
#
# TWO HALVES, ONE CLASS:
#   * Builder half (from Toy::Json) -- an ordered JSON-OBJECT builder with
#     the `j_*` instance appenders and `tj_*` class escapers. Used by toy's
#     tep-free event instrumentation.
#   * Encoder/decoder half (from Tep::Json) -- `escape/quote/hex2`, the
#     `encode_pair_*` / `from_*` encoders, and the flat-key decoders
#     (`get_str/get_int/get_float/get_int_array/has_key?`) with their
#     hand-rolled walker. Used by tep's JSON-over-HTTP request handling.
#
# The two halves share NO method or parameter names (builder: jk/jv/jchild;
# encoder/decoder: s/key/k/v), so the union is collision-safe under Spinel's
# whole-program inference. See docs/spinel-discipline.md.
#
# SPINEL NAMING DISCIPLINE (load-bearing -- do NOT "tidy" the names). Spinel
# keys whole-program inference partly on method- and parameter-NAMES: a
# builder method `num(key, value)` whose `value` is numeric-poly silently
# widens every other `value` param in the consuming program to poly --
# corrupting compute even when the gem is merely required and never called.
# Every name below is preserved verbatim from its donor for exactly this
# reason. In particular `escape` (encoder half) and `tj_escape` (builder
# half) are kept as SEPARATE byte-identical methods rather than delegating:
# tep's get_float comment documents that factoring through a shared helper
# mis-widened a param. Independence is the safe choice.
module SpinelKit
  class Json
    # ================================================================
    # Builder half (ported from Toy::Json). Mutating appenders, NOT
    # method-chaining (chaining is a Spinel poly-degradation risk).
    #
    #   j = SpinelKit::Json.new
    #   j.j_str("kind", "run_start")
    #   j.j_num("t", now)                 # int OR float via .to_s
    #   host = SpinelKit::Json.new
    ...
    def j_str(jk, jv)
      j_comma
      @j_buf = @j_buf + SpinelKit::Json.tj_quote(jk) + ":" + SpinelKit::Json.tj_quote(jv)
    end

    # Append `"jk":<number>` -- `jv.to_s` covers Integer ("5") and Float
    # ("1.5"). JSON-numeric (no quotes). For hardcoded literals prefer
    # j_raw (so Spinel's Float#to_s formatting can never drift the bytes).
    def j_num(jk, jv)
      j_comma
      @j_buf = @j_buf + SpinelKit::Json.tj_quote(jk) + ":" + jv.to_s
    end

    # Append `"jk":true|false`.
    def j_bool(jk, jv)
      j_comma
      @j_buf = @j_buf + SpinelKit::Json.tj_quote(jk) + ":" + (jv ? "true" : "false")
    end

    # Append `"jk":<already-encoded JSON>` -- for arrays / numeric literals.
    def j_raw(jk, jraw)
      j_comma
      @j_buf = @j_buf + SpinelKit::Json.tj_quote(jk) + ":" + jraw
    end

    # Append `"jk":<nested object>` from another SpinelKit::Json builder.
    def j_obj(jk, jchild)
      j_comma
      @j_buf = @j_buf + SpinelKit::Json.tj_quote(jk) + ":" + jchild.j_dump
    end
    ...
    # ---- string escaping (mirrors `escape` below; proven byte-identical) ----

    # Wrap a string in JSON quotes, escaping its body.
    def self.tj_quote(tjs)
      "\"" + SpinelKit::Json.tj_escape(tjs) + "\""
    end
    ...
    def self.tj_escape(tjs)
      ...
      while tji < tjn
        ...
        if tjc == "\""
          ...
          tjb = tjc.getbyte(0)
          tjout = tjout + "\\u00" + SpinelKit::Json.tj_hex2(tjb)
        else
          tjout = tjout + tjc
        end
        tji = tji + 1
    ...
    # ================================================================
    # Encoder/decoder half (ported from Tep::Json). A batteries-included
    # for JSON-over-HTTP shim, not a full JSON library. Compose objects
    # in user code by concatenation:
    #
    ...
    #
    # `get_str(s, key)` finds the entry for `key` in the top-level object
    # literal `s` and returns its value as a string. Returns "" when `key`
    # is absent or the value isn't a string. Same shape for `get_int`.
    # `has_key?(s, key)` returns a boolean independent of value type. The
    # parser is a hand-rolled state machine that walks one `{ "k": <value>,
    # ... }` pair at a time, skipping over any value (including nested
    # objects / arrays) it doesn't need. Strings inside values are honoured
    # for escape sequences so that `\"` doesn't terminate the string and
    # corrupt the walk.
    ...
    # boundaries), then String#to_f on the substring. Inlined rather than
    # factored into a parse_float_value helper because spinel's type
    # inference mis-widens `s` to int through the indirection ("cannot
    # resolve call to 'length' on int" + the downstream skip_ws/skip_value
    # pointer-vs-int conversion errors).
